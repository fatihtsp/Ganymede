...
type
  ...
  { TGnyCallback }
  TGnyCallback<T> = record
    Callback: T;
    UserData: Pointer;
    function IsAssigned(): Boolean;
  end;

  { TGnyCaptureConsoleCallback }
  TGnyCaptureConsoleCallback = reference to procedure(const ALine: string; const AUserData: Pointer);

  { TGnyStatusCallback }
  TGnyStatusCallback = reference to procedure(const AText: string; const AUserData: Pointer);

  { TGnyVersionInfo }
  TGnyVersionInfo = record
    Major: Word;
    Minor: Word;
    Patch: Word;
    Build: Word;
  ...
  public
    ...
    class procedure MsgBox(const ATitle, AMessage: string; const AType: TGnyMsgBoxType = pxlMsgInfo); static;
    ...
    class procedure CaptureConsoleOutput(const ATitle: string; const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TGnyCaptureConsoleCallback); static;
    class procedure CaptureZigConsolePTY(const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TGnyCaptureConsoleCallback); static;
    ...
    class function  GetVersionInfo(out AVersionInfo: TGnyVersionInfo; const AFilePath: string = ''): Boolean; static;
  ...
  TGnyError = record
    Range: TGnySourceRange;
    Severity: TGnyErrorSeverity;
    Code: string;
    Message: string;
    Related: TArray<TGnyErrorRelated>;
  ...
  { TGnyErrorNotify — callback fired each time an error is added }
  TGnyErrorNotify = reference to procedure(const AError: TGnyError);

  { TGnyErrors }
  TGnyErrors = class
  private
    FItems: TList<TGnyError>;
    FMaxErrors: Integer;
    FOnAdd: TGnyErrorNotify;
  ...
  public
    ...
    procedure Add(
      ...
      const ASeverity: TGnyErrorSeverity;
      const ACode: string;
    ...
    function GetItems(): TList<TGnyError>;
    function GetMaxErrors(): Integer;
    procedure SetMaxErrors(const AMaxErrors: Integer);
    function ToString(): string; override;

    property OnAdd: TGnyErrorNotify read FOnAdd write FOnAdd;
  end;

  { TGnyBaseObject }
  TGnyBaseObject = class
  protected
    FErrors: TGnyErrors;
    FOwnsErrors: Boolean;
    FStatusCallback: TGnyCallback<TGnyStatusCallback>;
  public
    constructor Create(); virtual;
    destructor Destroy(); override;
    ...
    function  GetStatusCallback(): TGnyStatusCallback;
    procedure SetStatusCallback(const ACallback: TGnyStatusCallback; const AUserData: Pointer = nil); virtual;
    procedure SetErrors(const AErrors: TGnyErrors); virtual;
    function  GetErrors(): TGnyErrors;
    procedure PropagateErrors(const AChild: TGnyBaseObject);
  end;

  { TGnyCommandBuilder }
  TGnyCommandBuilder = class(TGnyBaseObject)
  private
    FParams: TStringList;
  public
...
function TGnyCallback<T>.IsAssigned(): Boolean;
begin
...
class function TGnyUtils.EnableVirtualTerminalProcessing(): Boolean;
var
  HOut: THandle;
  LMode: DWORD;
begin
...
class procedure TGnyUtils.InitConsole();
begin
...
class procedure TGnyUtils.FailIf(const Cond: Boolean; const Msg: string; const AArgs: array of const);
begin
  if Cond then
    raise Exception.CreateFmt(Msg, AArgs);
end;

class function TGnyUtils.GetTickCount(): DWORD;
begin
  Result := WinApi.Windows.GetTickCount();
end;

class function TGnyUtils.GetTickCount64(): UInt64;
begin
  Result := WinApi.Windows.GetTickCount64();
end;

class function TGnyUtils.HasConsole(): Boolean;
begin
  Result := Boolean(GetConsoleWindow() <> 0);
end;

class procedure TGnyUtils.ClearToEOL();
begin
  if not HasConsole() then Exit;
  Write(#27'[0K');
end;

class procedure TGnyUtils.ClearScreen();
begin
  if not HasConsole() then Exit;
  Write(#27'[2J' + #27'[3J' + #27'[H');
end;

class procedure TGnyUtils.Print();
begin
  Print('');
end;

class procedure TGnyUtils.PrintLn();
begin
  PrintLn('');
end;

class procedure TGnyUtils.Print(const AText: string);
begin
  if not HasConsole() then Exit;
  Write(AText + COLOR_RESET);
end;

class procedure TGnyUtils.Print(const AText: string; const AArgs: array of const);
begin
  if not HasConsole() then Exit;
  Write(Format(AText, AArgs) + COLOR_RESET);
end;

class procedure TGnyUtils.PrintLn(const AText: string);
begin
  if not HasConsole() then Exit;
  WriteLn(AText + COLOR_RESET);
end;

class procedure TGnyUtils.PrintLn(const AText: string; const AArgs: array of const);
begin
  if not HasConsole() then Exit;
  WriteLn(Format(AText, AArgs) + COLOR_RESET);
end;

class procedure TGnyUtils.MsgBox(const ATitle, AMessage: string;
  const AType: TGnyMsgBoxType);
var
  LFlags: UINT;
begin
  case AType of
    pxlMsgInfo:    LFlags := MB_OK or MB_ICONINFORMATION;
    pxlMsgWarning: LFlags := MB_OK or MB_ICONWARNING;
    pxlMsgError:   LFlags := MB_OK or MB_ICONERROR;
  else
    LFlags := MB_OK;
  end;
...
class function TGnyUtils.RGB(const AR, AG, AB: Byte): string;
begin
  Result := #27'[38;2;' + IntToStr(AR) + ';' + IntToStr(AG) + ';' + IntToStr(AB) + 'm';
end;

class function TGnyUtils.Pause(const AMsg, AQuit: string): Boolean;
var
  LInput: string;
begin
  Result := False;
...
class function TGnyUtils.AsUTF8(const AValue: string; ALength: PCardinal): Pointer;
begin
...
class function TGnyUtils.ToAnsi(const AValue: string): AnsiString;
var
  LBytes: TBytes;
begin
...
class function TGnyUtils.FromUtf8(const APtr: PAnsiChar): string;
begin
  if APtr = nil then
    Result := ''
  else
...
class procedure TGnyUtils.ProcessMessages();
var
  LMsg: TMsg;
begin
...
class function TGnyUtils.RunPE(const AExe, AParams, AWorkDir: string; const AWait: Boolean; const AShowCmd: Word): Cardinal;
var
  LAppPath: string;
  LCmd: UnicodeString;
  LSI: STARTUPINFOW;
...
class function TGnyUtils.WindowsPathToWSL(const APath: string): string;
var
  LFullPath: string;
  LDrive: Char;
begin
...
class function TGnyUtils.RunElf(const AElf, AWorkDir: string): Cardinal;
var
  LWslPath: string;
  LCmd: UnicodeString;
  LSI: STARTUPINFOW;
...
class procedure TGnyUtils.CaptureConsoleOutput(const ATitle: string; const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TGnyCaptureConsoleCallback);
const
  CReadBuffer = 1024 * 2;
var
...
class procedure TGnyUtils.CaptureZigConsolePTY(const ACommand: PChar; const AParameters: PChar; const AWorkDir: string; var AExitCode: DWORD; const AUserData: Pointer; const ACallback: TGnyCaptureConsoleCallback);
const
  CReadBuffer = 4096;
var
...
class function TGnyUtils.CreateProcessWithPipes(const AExe, AParams, AWorkDir: string; out AStdinWrite: THandle; out AStdoutRead: THandle; out AProcessHandle: THandle; out AThreadHandle: THandle): Boolean;
var
...
class function TGnyUtils.CreateDirInPath(const AFilename: string): Boolean;
var
  LPath: string;
begin
...
class procedure TGnyUtils.CopyFilePreservingEncoding(const ASourceFile, ADestFile: string);
var
  LSourceBytes: TBytes;
begin
...
class function TGnyUtils.DetectFileEncoding(const AFilePath: string): TEncoding;
var
  LBytes: TBytes;
  LEncoding: TEncoding;
begin
...
class function TGnyUtils.EnsureBOM(const AText: string): string;
const
  UTF16_BOM = #$FEFF;
begin
  Result := AText;
...
class function TGnyUtils.EscapeString(const AText: string): string;
var
  LI: Integer;
  LChar: Char;
  LNextChar: Char;
...
class function TGnyUtils.StripAnsi(const AText: string): string;
var
...
class function TGnyUtils.ExtractAnsiCodes(const AText: string): string;
var
  LResult: TStringBuilder;
  LIdx: Integer;
  LLen: Integer;
...
class function TGnyUtils.GetVersionInfo(out AVersionInfo: TGnyVersionInfo; const AFilePath: string): Boolean;
var
  LFileName: string;
  LInfoSize: DWORD;
  LHandle: DWORD;
...
class function TGnyUtils.GetModuleVersionString(const AModule: HMODULE): string;
var
  LPath: array[0..MAX_PATH] of Char;
  LInfo: TGnyVersionInfo;
begin
...
class function TGnyUtils.IsValidWin64PE(const AFilePath: string): Boolean;
var
  LFile: TFileStream;
...
class procedure TGnyUtils.UpdateIconResource(const AExeFilePath, AIconFilePath: string);
type
...
class procedure TGnyUtils.UpdateVersionInfoResource(const PEFilePath: string; const AMajor, AMinor, APatch: Word; const AProductName, ADescription, AFilename, ACompanyName, ACopyright: string; const AURL: string);
type
...
class function TGnyUtils.ResourceExist(const AResName: string): Boolean;
begin
  Result := Boolean((FindResource(HInstance, PChar(AResName), RT_RCDATA) <> 0));
end;

class function TGnyUtils.AddResManifestFromResource(const AResName: string; const AModuleFile: string; ALanguage: Integer): Boolean;
var
  LHandle: THandle;
...
class procedure TGnyUtils.UpdateRCDataResource(const AExeFilePath: string;
...
class function TGnyUtils.GetFileSHA256(const APath: string): string;
begin
  Result := THashSHA2.GetHashStringFromFile(APath).ToLower();
end;

class function TGnyUtils.GetRelativePath(const ABasePath, AFullPath: string): string;
var
  LBasePath: string;
  LFullPath: string;
  LBaseLen: Integer;
...
class function TGnyUtils.NormalizePath(const APath: string): string;
begin
  Result := APath.Replace(PathDelim, '/');
end;

class function TGnyUtils.DisplayPath(const APath: string): string;
begin
  Result := TPath.GetFullPath(APath).Replace('\', '/');
end;

class function TGnyUtils.GetEnv(const AName: string): string;
begin
  Result := GetEnvironmentVariable(AName);
end;

class procedure TGnyUtils.SetEnv(const AName: string; const AValue: string);
begin
  SetEnvironmentVariable(PChar(AName), PChar(AValue));
end;

class function TGnyUtils.HasEnv(const AName: string): Boolean;
begin
  Result := not GetEnv(AName).IsEmpty();
end;

class function TGnyUtils.RunFromIDE(): Boolean;
begin
  Result := HasEnv('BDS');
end;

class function TGnyUtils.CountLines(
...
{ TGnySourceRange }
procedure TGnySourceRange.Clear();
begin
  Filename := '';
  StartLine := 0;
  StartColumn := 0;
...
function TGnySourceRange.IsEmpty(): Boolean;
begin
  Result := (StartLine = 0) and (StartColumn = 0);
end;

function TGnySourceRange.ToPointString(): string;
begin
  if IsEmpty() then
    Result := ''
  else
...
function TGnySourceRange.ToRangeString(): string;
begin
  if IsEmpty() then
    Result := ''
...
function TGnyError.GetSeverityString(): string;
begin
  case Severity of
...
function TGnyError.ToIDEString(): string;
begin
...
function TGnyError.ToFullString(): string;
var
...
constructor TGnyErrors.Create();
begin
  inherited;

  FItems := TList<TGnyError>.Create();
  FMaxErrors := DEFAULT_MAX_ERRORS;
end;

destructor TGnyErrors.Destroy();
begin
  FItems.Free();
...
function TGnyErrors.CountErrors(): Integer;
var
  LError: TGnyError;
begin
  Result := 0;
...
procedure TGnyErrors.AddRelated(
  const ARange: TGnySourceRange;
  const AMessage: string
);
var
  LError: TGnyError;
  LRelated: TGnyErrorRelated;
  LLen: Integer;
begin
...
function TGnyErrors.HasHints(): Boolean;
var
  LError: TGnyError;
begin
  Result := False;
...
function TGnyErrors.HasWarnings(): Boolean;
var
  LError: TGnyError;
begin
  Result := False;
...
function TGnyErrors.HasErrors(): Boolean;
var
  LError: TGnyError;
begin
  Result := False;
...
function TGnyErrors.HasFatal(): Boolean;
var
  LError: TGnyError;
begin
  Result := False;
...
function TGnyErrors.Count(): Integer;
begin
  Result := FItems.Count;
end;

function TGnyErrors.ErrorCount(): Integer;
begin
  Result := CountErrors();
end;

function TGnyErrors.WarningCount(): Integer;
var
  LError: TGnyError;
begin
  Result := 0;
...
function TGnyErrors.ReachedMaxErrors(): Boolean;
begin
  Result := CountErrors() >= FMaxErrors;
end;

procedure TGnyErrors.Clear();
begin
  FItems.Clear();
end;

procedure TGnyErrors.TruncateTo(const ACount: Integer);
begin
  while FItems.Count > ACount do
    FItems.Delete(FItems.Count - 1);
end;

function TGnyErrors.GetItems(): TList<TGnyError>;
begin
  Result := FItems;
end;

function TGnyErrors.GetMaxErrors(): Integer;
begin
  Result := FMaxErrors;
end;

procedure TGnyErrors.SetMaxErrors(const AMaxErrors: Integer);
begin
  FMaxErrors := AMaxErrors;
end;

function TGnyErrors.ToString(): string;
var
  LBuilder: TStringBuilder;
  LI: Integer;
begin
...
constructor TGnyBaseObject.Create();
begin
  inherited;
  FErrors := TGnyErrors.Create();
  FOwnsErrors := True;
end;

destructor TGnyBaseObject.Destroy();
begin
...
function TGnyBaseObject.Dump(const AId: Integer): string;
begin
  Result := '';
end;

procedure TGnyBaseObject.InitConfig();
begin
end;

procedure TGnyBaseObject.LoadConfig(const AFilename: string);
begin
end;

procedure TGnyBaseObject.SaveConfig(const AFilename: string);
begin
end;


procedure TGnyBaseObject.Status(const AText: string);
begin
  if FStatusCallback.IsAssigned() then
    FStatusCallback.Callback(AText, FStatusCallback.UserData);
end;

procedure TGnyBaseObject.Status(const AText: string; const AArgs: array of const);
begin
  Status(Format(AText, AArgs));
end;

procedure TGnyBaseObject.Status(const AEnabled: Boolean; const AText: string; const AArgs: array of const);
begin
  if AEnabled then
    Status(Format(AText, AArgs));
end;

function TGnyBaseObject.GetStatusCallback(): TGnyStatusCallback;
begin
  Result := FStatusCallback.Callback;
end;

procedure TGnyBaseObject.SetStatusCallback(const ACallback: TGnyStatusCallback; const AUserData: Pointer);
begin
  FStatusCallback.Callback := ACallback;
  FStatusCallback.UserData := AUserData;
end;

procedure TGnyBaseObject.SetErrors(const AErrors: TGnyErrors);
begin
...
function TGnyBaseObject.GetErrors(): TGnyErrors;
begin
  Result := FErrors;
end;

procedure TGnyBaseObject.PropagateErrors(const AChild: TGnyBaseObject);
begin
...
constructor TGnyCommandBuilder.Create();
begin
  inherited;
...
destructor TGnyCommandBuilder.Destroy();
begin
  FreeAndNil(FParams);
...
procedure TGnyCommandBuilder.Clear();
begin
  FParams.Clear();
end;

procedure TGnyCommandBuilder.AddParam(const AParam: string);
begin
  if AParam <> '' then
    FParams.Add(AParam);
end;

procedure TGnyCommandBuilder.AddParam(const AFlag, AValue: string);
begin
  if AFlag <> '' then
  begin
    if AValue <> '' then
...
procedure TGnyCommandBuilder.AddQuotedParam(const AFlag, AValue: string);
begin
  if AValue = '' then
    Exit;
...
procedure TGnyCommandBuilder.AddQuotedParam(const AValue: string);
begin
  AddQuotedParam('', AValue);
end;

procedure TGnyCommandBuilder.AddFlag(const AFlag: string);
begin
  if AFlag <> '' then
    FParams.Add(AFlag);
end;

function TGnyCommandBuilder.Dump(const AId: Integer): string;
var
  LI: Integer;
begin
...
function TGnyCommandBuilder.GetParamCount(): Integer;
begin
...
begin
  ...
  TGnyUtils.InitConsole();
  {$IFDEF VPR_LEAK_TRACK}
  TGnyBaseObject.InitLeakTracking();
  {$ENDIF}
end;
...
begin
  {$IFDEF VPR_LEAK_TRACK}
  TGnyBaseObject.FinalizeLeakTracking();
  {$ENDIF}
end;
