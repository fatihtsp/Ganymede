...
type
  ...
  //============================================================================
  // TGnyNativeDebugClient - DAP client that connects to a Viper DAP server
  // over TCP. Sends DAP requests, receives responses and events.
  // Adapted from Myra.Debug.TMyraDebug (pipe transport → TCP transport).
  //============================================================================

  { TGnyNativeDebugClient }
  TGnyNativeDebugClient = class(TGnyBaseObject)
  private
    FSocket: TSocket;
...
constructor TGnyNativeDebugClient.Create();
begin
  inherited Create();
...
destructor TGnyNativeDebugClient.Destroy();
begin
  Disconnect();
  inherited Destroy();
end;

function TGnyNativeDebugClient.GetNextSeq(): Integer;
begin
  Result := FNextSeq;
  Inc(FNextSeq);
end;

procedure TGnyNativeDebugClient.SetError(const AError: string);
begin
  FLastError := AError;
end;

function TGnyNativeDebugClient.HasError(): Boolean;
begin
  Result := FLastError <> '';
end;

function TGnyNativeDebugClient.GetLastError(): string;
begin
  Result := FLastError;
end;

function TGnyNativeDebugClient.GetState(): TDAPClientState;
begin
  Result := FState;
end;

function TGnyNativeDebugClient.GetCurrentThreadId(): Integer;
begin
...
function TGnyNativeDebugClient.Connect(const AHost: string; const APort: Integer): Boolean;
var
  LWSAData: TWSAData;
  LAddr: TSockAddrIn;
  LHostEnt: PHostEnt;
...
procedure TGnyNativeDebugClient.Disconnect();
begin
  if FSocket <> INVALID_SOCKET then
  begin
    closesocket(FSocket);
...
function TGnyNativeDebugClient.SendRaw(const AData: string): Boolean;
var
  LBytes: TBytes;
  LSent: Integer;
  LTotal: Integer;
...
procedure TGnyNativeDebugClient.SendDAPMessage(const AJson: string);
var
  LMsg: string;
begin
  ...
  if FVerboseLogging then
    TGnyUtils.PrintLn('[DAP-C] >> ' + AJson);
  SendRaw(LMsg);
end;

function TGnyNativeDebugClient.ReadDAPMessage(out AJson: string;
...
function TGnyNativeDebugClient.SendRequest(const ACommand: string;
...
procedure TGnyNativeDebugClient.ProcessEvent(const AEvent: TJSONObject);
var
  LEventName: string;
  LBody: TJSONObject;
  LReason: string;
...
procedure TGnyNativeDebugClient.ProcessPendingEvents(const ATimeoutMS: Integer);
var
  LJson: string;
  LObj: TJSONObject;
  LMsgType: string;
...
function TGnyNativeDebugClient.Initialize(): Boolean;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
begin
  ...
  LArgs.AddPair('clientID', 'viper-test');
  LArgs.AddPair('adapterID', 'viper');
...
function TGnyNativeDebugClient.Launch(const AProgram: string;
...
function TGnyNativeDebugClient.ConfigurationDone(): Boolean;
var
  LResponse: TJSONObject;
begin
  Result := False;
...
function TGnyNativeDebugClient.SetBreakpoints(const ASourcePath: string;
...
function TGnyNativeDebugClient.Continue(): Boolean;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
begin
...
function TGnyNativeDebugClient.StepOver(): Boolean;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
begin
...
function TGnyNativeDebugClient.StepIn(): Boolean;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
begin
...
function TGnyNativeDebugClient.StepOut(): Boolean;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
begin
...
function TGnyNativeDebugClient.GetCallStack(const AThreadId: Integer): TArray<TDAPClientStackFrame>;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
  LBody: TJSONObject;
...
function TGnyNativeDebugClient.GetScopes(const AFrameId: Integer): TArray<TDAPClientScope>;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
  LBody: TJSONObject;
...
function TGnyNativeDebugClient.GetVariables(const AVariablesReference: Integer): TArray<TDAPClientVariable>;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
  LBody: TJSONObject;
...
function TGnyNativeDebugClient.Evaluate(const AExpression: string;
...
function TGnyNativeDebugClient.DisconnectDAP(): Boolean;
var
  LArgs: TJSONObject;
  LResponse: TJSONObject;
begin
