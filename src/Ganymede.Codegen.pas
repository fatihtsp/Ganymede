...
type
  ...
  TJITCodeGenData = record
    TextSection: TMemoryStream;
    FuncOffsets: TArray<Cardinal>;
    ImportFixups: TList<TPair<Cardinal, Integer>>;
    CallFixups: TList<TPair<Cardinal, Integer>>;
    DataFixups: TList<TPair<Cardinal, Integer>>;
    GlobalFixups: TList<TPair<Cardinal, Integer>>;
    FuncAddrFixups: TList<TPair<Cardinal, Integer>>;
  end;
  PJITCodeGenData = ^TJITCodeGenData;

  { TCodegen }
  TCodegen = class(TGnyBaseObject)
  protected
    FOutputPath: string;
    FOutputType: TOutputType;
    FSubsystem: TSubsystem;
    FData: TDataBuilder;
    FGlobals: TDataBuilder;
    FImports: TImportBuilder;
    FExports: TExportBuilder;
    FCode: TCodeBuilder;
    FSSADump: string;
    FDumpIR: Boolean;
    ...
    FTextRVA: Cardinal;        // .text section RVA (saved for .vdbg emission)
  ...
  private
    ...
    // SSA-to-x64 emission (moved from TSSABuilder)
    function ResolveSSAOperand(
      const AOp: TSSAOperand;
      const ALocalHandles: TDictionary<string, TLocalHandle>;
      const AVarTemps: TDictionary<string, TTempHandle>
    ): TOperand;
    function StoreSSAVarValue(
      const ADest: TSSAVar;
      const AValue: TOperand;
      const ALocalHandles: TDictionary<string, TLocalHandle>;
      const AVarTemps: TDictionary<string, TTempHandle>
    ): TTempHandle;
    procedure EmitSSACall(
      const AInstr: TSSAInstr;
      const ALocalHandles: TDictionary<string, TLocalHandle>;
      const AVarTemps: TDictionary<string, TTempHandle>
    );
    ...
    procedure EmitSSAToCode(const ASSA: TSSABuilder);
  ...
  public
    ...
    function TargetDll(const APath: string): TCodegen;
    function TargetLib(const APath: string): TCodegen;
    function TargetObj(const APath: string): TCodegen;

    // Static Linking
    function AddLib(const APath: string): TCodegen;
    function AddObj(const APath: string): TCodegen;
    function AddLibPath(const APath: string): TCodegen;
    ...
    function GetSubsystem(): TSubsystem;
    function GetData(): TDataBuilder;
    function GetGlobals(): TDataBuilder;
    function GetImports(): TImportBuilder;
    function GetExports(): TExportBuilder;
    function GetCode(): TCodeBuilder;
    ...
    function TargetExe(const APath: string; const ASubsystem: TSubsystem = ssConsole): TCodegen; virtual;

    // Build
    function BuildToMemory(): TBytes; virtual;
    function BuildJIT(): TJIT; virtual;
    ...
    // Compile pipeline
    procedure Compile(const AIR: TIR; const AStatusCallback: TGnyStatusCallback = nil; const AUserData: Pointer = nil);
  end;
...
//==============================================================================
// TCodegen - Constructor/Destructor (from TBackend)
//==============================================================================
...
begin
  ...
  FData := TDataBuilder.Create();
  FGlobals := TDataBuilder.Create();
  FImports := TImportBuilder.Create();
  FCode := TCodeBuilder.Create();
  FExports := TExportBuilder.Create(FCode);
...
//==============================================================================
// TCodegen - Target Configuration (from TBackend)
//==============================================================================

function TCodegen.TargetDll(const APath: string): TCodegen;
begin
  FOutputPath := APath;
  FOutputType := otDll;
  FSubsystem := ssConsole;
  FOutputPath := TPath.ChangeExtension(FOutputPath, '.dll');
  Result := Self;
end;

function TCodegen.TargetLib(const APath: string): TCodegen;
begin
  FOutputPath := APath;
  FOutputType := otLib;
  FSubsystem := ssConsole;
  FOutputPath := TPath.ChangeExtension(FOutputPath, '.lib');
  Result := Self;
end;

function TCodegen.TargetObj(const APath: string): TCodegen;
begin
  FOutputPath := APath;
  FOutputType := otObj;
  FSubsystem := ssConsole;
  FOutputPath := TPath.ChangeExtension(FOutputPath, '.obj');
  Result := Self;
end;

//==============================================================================
// TCodegen - Static Linking (from TBackend)
//==============================================================================

function TCodegen.AddLib(const APath: string): TCodegen;
begin
...
function TCodegen.AddObj(const APath: string): TCodegen;
begin
...
function TCodegen.AddLibPath(const APath: string): TCodegen;
begin
...
//==============================================================================
// TCodegen - PreBuild/PostBuild (from TBackend)
//==============================================================================

procedure TCodegen.PreBuild();
begin
  TGnyUtils.CreateDirInPath(FOutputPath);
end;

procedure TCodegen.PostBuild();
begin
  // Emit .vdbg debug info sidecar file (PE debug mode only)
  if Assigned(FSourceMap) and (FOutputType in [otExe, otDll]) and
     (FSourceMap.GetEntryCount() > 0) then
  begin
    try
      FSourceMap.SaveToFile(ChangeFileExt(FOutputPath, '.vdbg'), FTextRVA);
      Status('Debug info written: %s',
        [ChangeFileExt(FOutputPath, '.vdbg').Replace('\', '/')]);
    except
      on E: Exception do
...
//==============================================================================
// TCodegen - Build (from TBackend)
//==============================================================================

function TCodegen.Build(): Boolean;
var
  LData: TBytes;
  LStream: TFileStream;
begin
  ...
  begin
    if Assigned(FErrors) then
      FErrors.Add(esError, ERR_BACKEND_OUTPUT_PATH, RSBackendOutputPath);
  ...
  begin
    if Assigned(FErrors) then
      FErrors.Add(esError, ERR_BACKEND_BUILD_FAILED, RSBackendBuildFailed);
  ...
  try
    ...
    begin
      if Assigned(FErrors) then
        FErrors.Add(esError, ERR_BACKEND_WRITE_FAILED, RSBackendWriteFailed, [E.Message]);
...
//==============================================================================
// TCodegen - Error Handling (from TBackend)
//==============================================================================

procedure TCodegen.SetErrors(const AErrors: TGnyErrors);
begin
...
//==============================================================================
// TCodegen - Getters/Setters (from TBackend)
//==============================================================================
...
function TCodegen.GetOutputType(): TOutputType;
begin
  Result := FOutputType;
end;

function TCodegen.GetSubsystem(): TSubsystem;
begin
  Result := FSubsystem;
end;

function TCodegen.GetData(): TDataBuilder;
begin
  Result := FData;
end;

function TCodegen.GetGlobals(): TDataBuilder;
begin
  Result := FGlobals;
end;

function TCodegen.GetImports(): TImportBuilder;
begin
  Result := FImports;
end;

function TCodegen.GetExports(): TExportBuilder;
begin
  Result := FExports;
end;

function TCodegen.GetCode(): TCodeBuilder;
begin
  Result := FCode;
end;

function TCodegen.GetSSADump(): string;
begin
  Result := FSSADump;
end;

procedure TCodegen.SetSSADump(const AValue: string);
begin
  FSSADump := AValue;
end;

function TCodegen.GetDumpIR(): Boolean;
begin
  Result := FDumpIR;
end;

procedure TCodegen.SetDumpIR(const AValue: Boolean);
begin
  FDumpIR := AValue;
end;

function TCodegen.GetOptimizationLevel(): Integer;
begin
  Result := FOptimizationLevel;
end;

procedure TCodegen.SetOptimizationLevel(const AValue: Integer);
begin
  FOptimizationLevel := AValue;
end;

procedure TCodegen.SetSourceMap(const ASourceMap: TSourceMap);
begin
  FSourceMap := ASourceMap;
end;

function TCodegen.GetSourceMap(): TSourceMap;
begin
  Result := FSourceMap;
end;

// TCodegen - Constructor/Destructor and Backend Methods
//==============================================================================

function TCodegen.TargetExe(const APath: string; const ASubsystem: TSubsystem): TCodegen;
begin
  FOutputPath := APath;
  FOutputType := otExe;
...
function TCodegen.BuildToMemory(): TBytes;
begin
  case FOutputType of
    otExe, otDll:
      Result := GeneratePE();
...
function TCodegen.Run(): Cardinal;
begin
  if FOutputType <> otExe then
    Exit(ERROR_BAD_FORMAT);

  try
    Result := TGnyUtils.RunPE(FOutputPath, '', ExtractFilePath(FOutputPath));
  except
    on E: Exception do
    begin
      if Assigned(FErrors) then
        FErrors.Add(esError, ERR_CODEGEN_RUN_FAILED, RSCodegenRunFailed, [E.Message]);
...
function TCodegen.GeneratePE(const AJITData: PJITCodeGenData): TBytes;
const
...
var
  ...
  LSEHHandlerImport: TImportHandle;          // Handle to __C_specific_handler import
  ...
  LImportVarFixups: TList<TPair<Cardinal, Integer>>;  // Code offset -> Import index (for MOV from IAT)
  ...
  LDisp: Int32;
  LDataIndex: Integer;
  LDataEntryRec: TDataEntry;
  LTargetRVA: Cardinal;
  LDataHandle: TDataHandle;
  ...
  //----------------------------------------------------------------------------
  procedure LoadOperandToReg(const AOp: TOperand; const AReg: Byte);
  begin
    case AOp.Kind of
      okImmediate:
        begin
      ...
      okImport:
        begin
          // MOV reg, [RIP+offset_to_IAT] — load address from IAT slot
          // 7 bytes: REX.W 8B ModRM disp32 (same layout as LEA)
          LImportVarFixups.Add(TPair<Cardinal, Integer>.Create(LTextSection.Size, AOp.ImportHandle.Index));
  ...
  procedure LoadOperandToXmm(const AOp: TOperand; const AXmm: Byte);
  begin
    case AOp.Kind of
      okImmediate:
        begin
  ...
  //----------------------------------------------------------------------------
  procedure LoadCallArgToReg(const AOp: TOperand; const AReg: Byte;
    const ATargetFuncIndex: Integer; const AArgIndex: Integer);
  var
    LNeedsAddress: Boolean;
    LLocalInfo: TLocalInfo;
    LTargetFunc: TFuncInfo;
  begin
    LNeedsAddress := False;
  ...
  //----------------------------------------------------------------------------
  procedure StoreOperandToStackArg(const AOp: TOperand; const AArgIndex: Integer);
  var
    LStackOffset: Int32;
  begin
  ...
  // Win64 ABI: Large structs (>8 bytes) are passed by pointer
  //----------------------------------------------------------------------------
  procedure StoreCallArgToStack(const AOp: TOperand; const AArgIndex: Integer;
...
begin
  ...
  LImportVarFixups := TList<TPair<Cardinal, Integer>>.Create();
  ...
  try
    ...
    LSEHHandlerImport := TImportHandle.Invalid();
    ...
    begin
      ...
      begin
        LFunc := FCode.GetFunc(LI);
        if LFunc.FuncName = 'Gny_InitExceptions' then
          LInitExceptionsIndex := LI
        else if LFunc.FuncName = 'Gny_SEHFilter' then
          LSEHFilterIndex := LI;
      end;
    end;
    ...
    begin
      ...
      if Assigned(FSourceMap) then
        FSourceMap.AddFunction(LI, LFunc.FuncName, Cardinal(LTextSection.Size), 0);
      ...
      if Assigned(FSourceMap) then
        FSourceMap.UpdateFunctionEnd(LI, Cardinal(LTextSection.Size));
    ...
    begin
      if Assigned(FErrors) then
        FErrors.Add(esError, ERR_CODEGEN_NO_CODE, RSCodegenNoCode);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          LExportName := TABIMangler.MangleFunctionWithLinkage(
    ...
    for LI := 0 to FCompilePublicGlobals.Count - 1 do
      LExportFuncs.Add(TPair<Integer, string>.Create(
        -(FCompilePublicGlobals[LI].Key + 1),
        FCompilePublicGlobals[LI].Value));
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // HandlerAddress: filter function RVA for __except, 0 for __finally
          // The filter (Gny_SEHFilter) captures hardware exception codes into TLS
          if LScope.ExceptLabel.Index >= 0 then
            LTextSection.WriteData(Cardinal(LFuncOffsets[LSEHFilterIndex]))   // __except via filter
          else
    ...
    begin
      ...
      for LI := 0 to LStaticLibPaths.Count - 1 do
        LLinker.AddLibraryFile(LStaticLibPaths[LI], GetDumpIR());
      ...
      for LI := 0 to FLinkedLibs.Count - 1 do
        LLinker.AddLibraryFile(FLinkedLibs[LI], GetDumpIR());
    ...
    FTextRVA := LTextRVA;  // Save for .vdbg debug info emission
    ...
    begin
      ...
      begin
        ...
        LTextSection.Position := LCodeOffset + 4 + (Cardinal(LJ) * 16) + 8;
        LTextSection.ReadData(LTargetRVA, SizeOf(Cardinal));
    ...
    for LI := 0 to LImportVarFixups.Count - 1 do
    begin
      LCodeOffset := LImportVarFixups[LI].Key;
      LImportIndex := LImportVarFixups[LI].Value;
      if LImportIndex >= LIATEntries.Count then
        Continue;
      LIATSlotOffset := LIATEntries[LImportIndex];
      LIATSlotRVA := LIDataRVA + LIATSlotOffset;
      ...
      LTextSection.Position := LCodeOffset + 3;  // disp32 at offset 3
      LTextSection.WriteData(LDisp);
    end;
...
// =============================================================================
// GenerateCOFF() and GenerateLib() implementation for Viper.Codegen.pas
...
function TCodegen.GenerateCOFF(): TBytes;
const
...
var
  ...
  procedure EmitSeta();
  begin
    EmitByte($0F); EmitByte($97); EmitModRM(3, 0, REG_RAX);
  end;

  procedure EmitSetae();
  begin
    EmitByte($0F); EmitByte($93); EmitModRM(3, 0, REG_RAX);
  end;

  procedure EmitSetnp();
  begin
    EmitByte($0F); EmitByte($9B); EmitModRM(3, 0, REG_RAX);
  end;

  procedure EmitSetpCl();
  begin
    EmitByte($0F); EmitByte($9A); EmitModRM(3, 0, REG_RCX);
  end;

  procedure EmitSetnpCl();
  begin
    EmitByte($0F); EmitByte($9B); EmitModRM(3, 0, REG_RCX);
  end;
...
begin
  ...
  try
    ...
    if LHasSEH then
    begin
      // Find Gny_InitExceptions function index
      for LI := 0 to FCode.GetFuncCount() - 1 do
      begin
        LFunc := FCode.GetFunc(LI);
        if LFunc.FuncName = 'Gny_InitExceptions' then
    ...
    begin
      ...
      // For entry point functions using SEH, call Gny_InitExceptions first
      if LHasSEH and (LInitExceptionsIndex >= 0) then
      begin
        if (LFunc.IsEntryPoint and (FOutputType <> otDll)) or
           (LFunc.IsDllEntry and (FOutputType = otDll)) then
...
function TCodegen.GenerateLib(): TBytes;
var
  LObjData: TBytes;
  LOutput: TMemoryStream;
  LFunc: TFuncInfo;
...
begin
  ...
  begin
    if Assigned(FErrors) then
      FErrors.Add(esError, ERR_CODEGEN_COFF_FAILED, RSCodegenCOFFFailed);
...
function TCodegen.BuildJIT(): TJIT;
var
  LJIT: TJITWin64;
  ...
  LHasStaticImports: Boolean;
  LExtTextOffset: Cardinal;
  LExtTextSize: Cardinal;
  LExtRDataOffset: Cardinal;
  LExtRDataSize: Cardinal;
  LExtDataOffset: Cardinal;
  LExtDataSize: Cardinal;
  LMergedBytes: TBytes;
...
begin
  ...
  //----------------------------------------------------------------------------
  // Collect static imports and run COFF linker
  //----------------------------------------------------------------------------
  LLinker := nil;
  LStaticImportIndices := TList<Integer>.Create();
  LStaticSymbolNames := TStringList.Create();
  ...
  // Scan imports for static (.lib) entries
  for LI := 0 to FImports.GetCount() - 1 do
  begin
    LEntry := FImports.GetEntryByIndex(LI);
    if LEntry.IsStatic then
  ...
  // Run COFF linker if we have static imports
  if LHasStaticImports then
  begin
    LLinker := TCOFFLinker.Create();
    LLinker.SetStatusCallback(FStatusCallback.Callback, FStatusCallback.UserData);
    ...
    // Get merged section sizes
    LExtTextSize := LLinker.GetMergedTextSize();
    LExtRDataSize := LLinker.GetMergedRDataSize();
    LExtDataSize := LLinker.GetMergedDataSize();

    Status('JIT: Linker merged %d bytes .text, %d bytes .rdata, %d bytes .data',
      [LExtTextSize, LExtRDataSize, LExtDataSize]);
  end;

  LJIT := TJITWin64.Create();
  try
    // Calculate memory layout: [Code][ExtText][Data][ExtRData][ExtData][Globals][IAT]
    ...
    // External code goes after user code, aligned to 16
    LExtTextOffset := (LCodeSize + 15) and not Cardinal(15);
    LDataOffset := LExtTextOffset + LExtTextSize;
    // External rdata goes after user data, aligned to 8
    LExtRDataOffset := (LDataOffset + LDataSize + 7) and not Cardinal(7);
    // External data goes after external rdata, aligned to 8
    LExtDataOffset := (LExtRDataOffset + LExtRDataSize + 7) and not Cardinal(7);
    LGlobalOffset := LExtDataOffset + LExtDataSize;
    ...
    // Copy merged sections from static linker
    if LHasStaticImports then
    begin
      LMergedBytes := LLinker.GetMergedText();
      if Length(LMergedBytes) > 0 then
        Move(LMergedBytes[0], (LCodeBase + LExtTextOffset)^, Length(LMergedBytes));

      LMergedBytes := LLinker.GetMergedRData();
      if Length(LMergedBytes) > 0 then
        Move(LMergedBytes[0], (LCodeBase + LExtRDataOffset)^, Length(LMergedBytes));

      LMergedBytes := LLinker.GetMergedData();
      if Length(LMergedBytes) > 0 then
        Move(LMergedBytes[0], (LCodeBase + LExtDataOffset)^, Length(LMergedBytes));
    end;
    ...
    begin
      LEntry := FImports.GetEntryByIndex(LI);
      if LEntry.HostAddr <> nil then
        LResolvedAddr := LEntry.HostAddr
      else if LEntry.IsStatic then
      begin
        // Resolve from linker — find symbol in merged sections
        LResolvedAddr := nil;
        if (LStaticResolved <> nil) and
           LStaticResolved.TryGetValue(LEntry.FuncName, LResolvedSym) then
        begin
          case LResolvedSym.SectionKind of
            lskText:  LResolvedAddr := LCodeBase + LExtTextOffset + LResolvedSym.OffsetInMerged;
            lskRData: LResolvedAddr := LCodeBase + LExtRDataOffset + LResolvedSym.OffsetInMerged;
            lskData:  LResolvedAddr := LCodeBase + LExtDataOffset + LResolvedSym.OffsetInMerged;
          end;
        end;
      end
      else
    ...
    //--------------------------------------------------------------------------
    // Apply COFF linker pending relocations (external code fixups)
    //--------------------------------------------------------------------------
    if LHasStaticImports and (LLinker <> nil) then
    begin
      LPendingRelocs := LLinker.GetPendingRelocations();
      ...
      begin
        ...
        // Calculate target offset in JIT flat memory
        case LResolvedSym.SectionKind of
          lskText:  LSymTargetOffset := LExtTextOffset + LResolvedSym.OffsetInMerged;
          lskRData: LSymTargetOffset := LExtRDataOffset + LResolvedSym.OffsetInMerged;
          lskData:  LSymTargetOffset := LExtDataOffset + LResolvedSym.OffsetInMerged;
        else
          Continue;
        end;

        // Calculate site offset in JIT flat memory
        case LPendReloc.SectionKind of
          lskText:  LSiteOffset := LExtTextOffset + LPendReloc.OffsetInMerged;
          lskRData: LSiteOffset := LExtRDataOffset + LPendReloc.OffsetInMerged;
          lskData:  LSiteOffset := LExtDataOffset + LPendReloc.OffsetInMerged;
        else
          Continue;
        end;
        ...
        begin
          // REL32 family: disp = target - (site + 4 + N) + addend
          LDisp := Int32(LSymTargetOffset) - Int32(LSiteOffset + 4 +
            Cardinal(LPendReloc.RelocationType - IMAGE_REL_AMD64_REL32)) +
            LPendReloc.Addend;
          PInt32(LCodeBase + LSiteOffset)^ := LDisp;
        end
        else if LPendReloc.RelocationType = IMAGE_REL_AMD64_ADDR64 then
        begin
          // ADDR64: 64-bit absolute address
          PUInt64(LCodeBase + LSiteOffset)^ :=
            NativeUInt(LCodeBase) + LSymTargetOffset;
        end
        else if LPendReloc.RelocationType = IMAGE_REL_AMD64_ADDR32NB then
        begin
          // ADDR32NB: not meaningful in JIT (no image base), treat as offset
          LDisp := Int32(LSymTargetOffset) + LPendReloc.Addend;
          PInt32(LCodeBase + LSiteOffset)^ := LDisp;
        end;
      end;

      Status('JIT: Applied %d pending relocations', [LPendingRelocs.Count]);
    end;
    ...
    if LHasStaticImports then
      Status('JIT: Generated %d bytes (code=%d, ext=%d, data=%d, globals=%d, IAT=%d)',
        [LTotalSize, LCodeSize, LExtTextSize + LExtRDataSize + LExtDataSize,
         LDataSize, LGlobalSize, LIATSize])
    else
...
//==============================================================================
// TCodegen - Compile (IR -> SSA -> x64)
//==============================================================================

procedure TCodegen.Compile(const AIR: TIR; const AStatusCallback: TGnyStatusCallback; const AUserData: Pointer);
var
  LI: Integer;
  LSize: Integer;
  LSSA: TSSABuilder;
...
begin
  ...
  begin
    if AIR.GetImport(LI).Linkage = plC then
      ...
      LImportName := TABIMangler.MangleFunctionWithLinkage(
  ...
  begin
    ...
    begin
      ...
      if LExpr.Kind = TIR.TIRExprKind.ekConstInt then
      begin
        case LSize of
      ...
      else if LExpr.Kind = TIR.TIRExprKind.ekConstFloat then
      begin
        case LSize of
...
//==============================================================================
// TCodegen - SSA Emission (moved from TSSABuilder)
//==============================================================================

function TCodegen.ResolveSSAOperand(
  const AOp: TSSAOperand;
  const ALocalHandles: TDictionary<string, TLocalHandle>;
  const AVarTemps: TDictionary<string, TTempHandle>
): TOperand;
var
  LLocalHandle: TLocalHandle;
  LTempHandle: TTempHandle;
  LVarKey: string;
begin
  case AOp.Kind of
    sokNone:
      Result := TOperand.None();
      
    sokImmediate:
      begin
        if AOp.ImmFloat <> 0 then
          Result := TOperand.FromImm(AOp.ImmFloat)
        else
          Result := TOperand.FromImm(AOp.ImmInt);
      end;
    ...
    sokVar:
      begin
        ...
        begin
          Result := TOperand.FromTemp(LTempHandle);
        end
        ...
        begin
          ...
          Result := TOperand.FromImm(Int64(0));
        end;
      end;
    ...
    sokData:
      begin
        ...
        Result := TOperand.FromData(FCompileStringHandles[AOp.DataIndex]);
      end;
      
    sokGlobal:
      begin
        // Global handle - use actual handle from FCompileGlobalHandles (goes to .data section)
        Result := TOperand.FromGlobal(FCompileGlobalHandles[AOp.GlobalIndex]);
      end;
      
    sokImport:
      begin
        // Import handle - use actual handle from FCompileImportHandles
        Result := TOperand.FromImport(FCompileImportHandles[AOp.ImportIndex]);
      end;
      
    sokFunc:
      begin
        // Function handle
        Result := TOperand.FromFunc(TFuncHandle.Invalid());
...
function TCodegen.StoreSSAVarValue(
  const ADest: TSSAVar;
  const AValue: TOperand;
  const ALocalHandles: TDictionary<string, TLocalHandle>;
  const AVarTemps: TDictionary<string, TTempHandle>
...
procedure TCodegen.EmitSSACall(
  const AInstr: TSSAInstr;
  const ALocalHandles: TDictionary<string, TLocalHandle>;
  const AVarTemps: TDictionary<string, TTempHandle>
);
var
  LArgs: TArray<TOperand>;
...
function TCodegen.EmitSSACallWithResult(
  const AInstr: TSSAInstr;
  const ALocalHandles: TDictionary<string, TLocalHandle>;
  const AVarTemps: TDictionary<string, TTempHandle>
): TTempHandle;
...
procedure TCodegen.EmitSSAIndirectCall(
  const AInstr: TSSAInstr;
  const ALocalHandles: TDictionary<string, TLocalHandle>;
  const AVarTemps: TDictionary<string, TTempHandle>
);
...
function TCodegen.EmitSSAIndirectCallWithResult(
  const AInstr: TSSAInstr;
  const ALocalHandles: TDictionary<string, TLocalHandle>;
  const AVarTemps: TDictionary<string, TTempHandle>
): TTempHandle;
...
begin
  ...
  if Length(LArgs) > 0 then
    Result := GetCode().CallIndirectFunc(LFuncPtr, LArgs)
  else
    Result := GetCode().CallIndirectFunc(LFuncPtr);
end;

procedure TCodegen.EmitSSAToCode(const ASSA: TSSABuilder);
var
  LI: Integer;
  LJ: Integer;
  LK: Integer;
  ...
  LLocalHandles: TDictionary<string, TLocalHandle>;
  LLabelHandles: TDictionary<Integer, TLabelHandle>;
  LVarTemps: TDictionary<string, TTempHandle>;
  LLocalHandle: TLocalHandle;
  LLabelHandle: TLabelHandle;
  LOp1: TOperand;
  LOp2: TOperand;
  LDestTemp: TTempHandle;
  // Exception scope handling
  LExcScope: TSSAExceptionScope;
  LTryBeginHandle: TLabelHandle;
  LTryEndHandle: TLabelHandle;
  LExceptHandle: TLabelHandle;
  LFinallyHandle: TLabelHandle;
  LEndHandle: TLabelHandle;
...
begin
  ...
  begin
    ...
    LLocalHandles := TDictionary<string, TLocalHandle>.Create();
    LLabelHandles := TDictionary<Integer, TLabelHandle>.Create();
    LVarTemps := TDictionary<string, TTempHandle>.Create();
    try
      //------------------------------------------------------------------------
      // Begin function
      //------------------------------------------------------------------------
          ...
          case LInstr.Kind of
            ...
            sikLoad:
              begin
                ...
                begin
                  LDestTemp := GetCode().LoadPtr(LOp1, LInstr.MemSize, LInstr.MemIsFloat);
                  LVarTemps.AddOrSetValue(LInstr.Dest.ToString(), LDestTemp);
                end;
              end;
            ...
            sikIndexAddr:
              begin
                ...
                LDestTemp := GetCode().OpAdd(LOp1, TOperand.FromTemp(LDestTemp));
                LVarTemps.AddOrSetValue(LInstr.Dest.ToString(), LDestTemp);
              end;
