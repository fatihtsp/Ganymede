...
type
  ...
  //----------------------------------------------------------------------------
  // TLinker - Base linker class with virtual interface
  //----------------------------------------------------------------------------

  { TLinker }
  TLinker = class(TGnyBaseObject)
  public
  ...
  //----------------------------------------------------------------------------
  // TCOFFLinker - COFF/PE static linker implementation

  //----------------------------------------------------------------------------
type
  { TCOFFLinker }
  TCOFFLinker = class(TLinker)
  private
...
//==============================================================================
// TLinker - Base (empty virtual stubs)
//==============================================================================
...
procedure TLinker.AddObjectFile(const APath: string);
begin
end;

procedure TLinker.AddLibraryFile(const APath: string; const ADebug: Boolean);
begin
end;

procedure TLinker.AddObjectFromMemory(const AData: TBytes; const AName: string);
begin
end;

procedure TLinker.Resolve(const ANeededSymbols: TStrings);
begin
end;

function TLinker.GetMergedText(): TBytes;
begin
  SetLength(Result, 0);
end;

function TLinker.GetMergedTextSize(): Cardinal;
begin
  Result := 0;
end;

function TLinker.GetMergedRData(): TBytes;
begin
  SetLength(Result, 0);
end;

function TLinker.GetMergedRDataSize(): Cardinal;
begin
  Result := 0;
end;

function TLinker.GetMergedData(): TBytes;
begin
  SetLength(Result, 0);
end;

function TLinker.GetMergedDataSize(): Cardinal;
begin
  Result := 0;
end;

function TLinker.GetMergedPData(): TBytes;
begin
  SetLength(Result, 0);
end;

function TLinker.GetMergedPDataSize(): Cardinal;
begin
  Result := 0;
end;

function TLinker.GetMergedXData(): TBytes;
begin
  SetLength(Result, 0);
end;

function TLinker.GetMergedXDataSize(): Cardinal;
begin
  Result := 0;
end;

function TLinker.GetResolvedSymbols(): TDictionary<string, TLinkerResolvedSymbol>;
begin
  Result := nil;
end;

function TLinker.GetUnresolvedSymbols(): TStringList;
begin
  Result := nil;
end;

function TLinker.GetPendingRelocations(): TList<TLinkerPendingReloc>;
begin
  Result := nil;
end;

function TLinker.GetObjectCount(): Integer;
begin
  Result := 0;
end;

function TLinker.GetSelectedObjectCount(): Integer;
begin
  Result := 0;
end;
...
destructor TCOFFLinker.Destroy();
var
...
function TCOFFLinker.ReadCOFFSymbolName(const AData: TBytes;
...
function TCOFFLinker.ParseCOFFObject(const AData: TBytes;
...
begin
  ...
  begin
    if Assigned(FErrors) then
      FErrors.Add(esError, ERR_LINKER_COFF_TOO_SMALL, RSLinkerCOFFTooSmall, [ASourcePath]);
  ...
  begin
    if Assigned(FErrors) then
      FErrors.Add(esError, ERR_LINKER_NOT_AMD64, RSLinkerNotAMD64, [ASourcePath, LMachine]);
...
procedure TCOFFLinker.ParseARLibrary(const AData: TBytes;
...
begin
  ...
  begin
    if Assigned(FErrors) then
      FErrors.Add(esError, ERR_LINKER_AR_TOO_SMALL, RSLinkerARTooSmall, [ASourcePath]);
  ...
  begin
    if Assigned(FErrors) then
      FErrors.Add(esError, ERR_LINKER_AR_BAD_SIG, RSLinkerARBadSig, [ASourcePath]);
...
function TCOFFLinker.ClassifySection(const ASectionName: string;
  const ACharacteristics: Cardinal): TLinkerSectionKind;
begin
...
procedure TCOFFLinker.BuildGlobalSymbolIndex();
var
  LI, LJ: Integer;
  LObj: TCOFFObject;
  LSym: TCOFFSymbol;
...
procedure TCOFFLinker.SelectObject(const AObjectIndex: Integer;
  const AWorkList: TList<Integer>);
var
  LObj: TCOFFObject;
  LSym: TCOFFSymbol;
...
function TCOFFLinker.AlignUp(const AValue: Cardinal;
...
procedure TCOFFLinker.MergeSelectedObjects();
var
  LI, LJ: Integer;
  LObj: TCOFFObject;
  LSect: TCOFFSection;
...
procedure TCOFFLinker.CollectRelocations();
var
  LI, LJ, LK, LM: Integer;
  LObj: TCOFFObject;
  LSect: TCOFFSection;
...
procedure TCOFFLinker.AddObjectFile(const APath: string);
var
  LData: TBytes;
  LObj: TCOFFObject;
begin
  ...
  begin
    if Assigned(FErrors) then
      FErrors.Add(esError, ERR_LINKER_OBJ_NOT_FOUND, RSLinkerObjNotFound, [APath]);
...
procedure TCOFFLinker.AddLibraryFile(const APath: string; const ADebug: Boolean);
var
  LData: TBytes;
begin
  if not FileExists(APath) then
  begin
    if Assigned(FErrors) then
      FErrors.Add(esError, ERR_LINKER_LIB_NOT_FOUND, RSLinkerLibNotFound, [APath]);
...
procedure TCOFFLinker.AddObjectFromMemory(const AData: TBytes;
  const AName: string);
var
  LObj: TCOFFObject;
begin
...
procedure TCOFFLinker.Resolve(const ANeededSymbols: TStrings);
var
  LI: Integer;
...
function TCOFFLinker.GetMergedText(): TBytes;
begin
...
function TCOFFLinker.GetMergedTextSize(): Cardinal;
begin
  Result := Cardinal(FMergedText.Size);
end;

function TCOFFLinker.GetMergedRData(): TBytes;
begin
...
function TCOFFLinker.GetMergedRDataSize(): Cardinal;
begin
  Result := Cardinal(FMergedRData.Size);
end;

function TCOFFLinker.GetMergedData(): TBytes;
begin
...
function TCOFFLinker.GetMergedDataSize(): Cardinal;
begin
  Result := Cardinal(FMergedData.Size);
end;

function TCOFFLinker.GetMergedPData(): TBytes;
begin
...
function TCOFFLinker.GetMergedPDataSize(): Cardinal;
begin
  Result := Cardinal(FMergedPData.Size);
end;

function TCOFFLinker.GetMergedXData(): TBytes;
begin
...
function TCOFFLinker.GetMergedXDataSize(): Cardinal;
begin
  Result := Cardinal(FMergedXData.Size);
end;

function TCOFFLinker.GetResolvedSymbols(): TDictionary<string, TLinkerResolvedSymbol>;
begin
  Result := FResolvedSymbols;
end;

function TCOFFLinker.GetUnresolvedSymbols(): TStringList;
begin
  Result := FUnresolvedSymbols;
end;

function TCOFFLinker.GetPendingRelocations(): TList<TLinkerPendingReloc>;
begin
  Result := FPendingRelocs;
end;

function TCOFFLinker.GetObjectCount(): Integer;
begin
  Result := FObjects.Count;
end;

function TCOFFLinker.GetSelectedObjectCount(): Integer;
var
  LI: Integer;
begin
  Result := 0;
