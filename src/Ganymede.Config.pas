...
type
  { TGnyConfig }
  TGnyConfig = class(TGnyBaseObject)
  private
    FToml:      TGnyToml;
    FLastError: string;
    FFilename:  string;
...
destructor TGnyConfig.Destroy();
begin
  Clear();
  inherited;
end;

procedure TGnyConfig.Clear();
begin
...
function TGnyConfig.GetLastError(): string;
begin
  Result := FLastError;
end;

function TGnyConfig.NavigateToTable(const AKeyPath: string;
  const ACreateMissing: Boolean; out ATable: TGnyToml;
  out AFinalKey: string): Boolean;
var
...
function TGnyConfig.GetValueAtPath(const AKeyPath: string;
  out AValue: TGnyTomlValue): Boolean;
var
  LTable:    TGnyToml;
  LFinalKey: string;
...
function TGnyConfig.LoadFromFile(const AFilename: string): Boolean;
begin
  Result := False;
  Clear();
  ...
  try
    FToml     := TGnyToml.FromFile(AFilename);
    FFilename := AFilename;
    Result    := True;
  except
    on E: Exception do
...
function TGnyConfig.LoadFromString(const ASource: string): Boolean;
begin
  Result := False;
  Clear();

  try
    FToml  := TGnyToml.FromString(ASource);
    Result := True;
  except
    on E: Exception do
    begin
...
function TGnyConfig.EscapeString(const AValue: string): string;
begin
  Result := AValue;
...
procedure TGnyConfig.SerializeValue(const AValue: TGnyTomlValue;
  const ABuilder: TStringBuilder);
var
...
procedure TGnyConfig.SerializeToml(const AToml: TGnyToml;
  const ABuilder: TStringBuilder; const AIndent: Integer;
  const ATablePath: string);
var
  LKeys:     TArray<string>;
...
function TGnyConfig.SaveToFile(const AFilename: string): Boolean;
var
...
begin
  ...
  try
    ...
    try
      TGnyUtils.CreateDirInPath(AFilename);
...
function TGnyConfig.HasKey(const AKeyPath: string): Boolean;
var
  LValue: TGnyTomlValue;
begin
...
function TGnyConfig.GetString(const AKeyPath: string;
  const ADefault: string): string;
var
  LValue: TGnyTomlValue;
begin
...
procedure TGnyConfig.SetString(const AKeyPath: string;
  const AValue: string);
var
  LTable:    TGnyToml;
  LFinalKey: string;
...
function TGnyConfig.GetInteger(const AKeyPath: string;
...
procedure TGnyConfig.SetInteger(const AKeyPath: string;
  const AValue: Int64);
var
  LTable:    TGnyToml;
  LFinalKey: string;
...
function TGnyConfig.GetFloat(const AKeyPath: string;
...
procedure TGnyConfig.SetFloat(const AKeyPath: string;
  const AValue: Double);
var
  LTable:    TGnyToml;
  LFinalKey: string;
...
function TGnyConfig.GetBoolean(const AKeyPath: string;
...
procedure TGnyConfig.SetBoolean(const AKeyPath: string;
...
function TGnyConfig.GetDateTime(const AKeyPath: string;
...
procedure TGnyConfig.SetDateTime(const AKeyPath: string;
...
begin
  ...
  if NavigateToTable(AKeyPath, True, LTable, LFinalKey) then
    LTable.SetValue(LFinalKey, TGnyTomlValue.CreateDateTime(AValue));
end;

// -- String Array -------------------------------------------------------------

function TGnyConfig.GetStringArray(const AKeyPath: string): TArray<string>;
var
  LValue: TGnyTomlValue;
  LArray: TGnyTomlArray;
  LI:     Integer;
...
procedure TGnyConfig.SetStringArray(const AKeyPath: string;
  const AValues: TArray<string>);
var
  LTable:    TGnyToml;
  LFinalKey: string;
...
function TGnyConfig.GetIntegerArray(const AKeyPath: string): TArray<Int64>;
var
  LValue: TGnyTomlValue;
  LArray: TGnyTomlArray;
  LI:     Integer;
...
procedure TGnyConfig.SetIntegerArray(const AKeyPath: string;
  const AValues: TArray<Int64>);
var
  LTable:    TGnyToml;
  LFinalKey: string;
...
function TGnyConfig.GetFloatArray(const AKeyPath: string): TArray<Double>;
var
  LValue: TGnyTomlValue;
  LArray: TGnyTomlArray;
  LI:     Integer;
...
procedure TGnyConfig.SetFloatArray(const AKeyPath: string;
  const AValues: TArray<Double>);
var
  LTable:    TGnyToml;
  LFinalKey: string;
...
function TGnyConfig.GetTableCount(const AKeyPath: string): Integer;
var
  LValue: TGnyTomlValue;
begin
  Result := 0;
...
function TGnyConfig.GetTableString(const AKeyPath: string;
  const AIndex: Integer; const AField: string;
  const ADefault: string): string;
var
  LValue:      TGnyTomlValue;
...
function TGnyConfig.GetTableInteger(const AKeyPath: string;
  const AIndex: Integer; const AField: string;
  const ADefault: Int64): Int64;
var
  LValue:      TGnyTomlValue;
...
function TGnyConfig.GetTableFloat(const AKeyPath: string;
  const AIndex: Integer; const AField: string;
  const ADefault: Double): Double;
var
  LValue:      TGnyTomlValue;
...
function TGnyConfig.GetTableBoolean(const AKeyPath: string;
  const AIndex: Integer; const AField: string;
  const ADefault: Boolean): Boolean;
var
  LValue:      TGnyTomlValue;
...
function TGnyConfig.AddTableEntry(const AKeyPath: string): Integer;
var
  LTable:    TGnyToml;
  LFinalKey: string;
  LArray:    TGnyTomlArray;
...
procedure TGnyConfig.SetTableString(const AKeyPath: string;
...
procedure TGnyConfig.SetTableInteger(const AKeyPath: string;
...
procedure TGnyConfig.SetTableFloat(const AKeyPath: string;
...
procedure TGnyConfig.SetTableBoolean(const AKeyPath: string;
...
function TGnyConfig.GetTableStringArray(const AKeyPath: string;
  const AIndex: Integer; const AField: string): TArray<string>;
var
  LValue:      TGnyTomlValue;
  LArray:      TGnyTomlArray;
...
procedure TGnyConfig.SetTableStringArray(const AKeyPath: string;
  const AIndex: Integer; const AField: string;
  const AValues: TArray<string>);
var
  LValue:      TGnyTomlValue;
...
function TGnyConfig.GetFileComment(): string;
begin
  if Assigned(FToml) then
...
procedure TGnyConfig.SetFileComment(const AComment: string);
begin
  if not Assigned(FToml) then
    FToml := TGnyToml.Create();
...
function TGnyConfig.GetTableComment(const AKeyPath: string;
  const AIndex: Integer): string;
var
  LValue: TGnyTomlValue;
  LArray: TGnyTomlArray;
...
procedure TGnyConfig.SetTableComment(const AKeyPath: string;
  const AIndex: Integer; const AComment: string);
var
  LValue: TGnyTomlValue;
  LArray: TGnyTomlArray;
