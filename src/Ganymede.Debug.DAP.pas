...
type
  ...
  // This is the server side — VS Code connects to us.
  //============================================================================

  { TDAPServer }
  TDAPServer = class(TGnyBaseObject)
  private
    ...
    // Launch configuration fields (extracted from DAP launch request)
    FProgram: string;            // Path to .exe, .dll, or .vpr
    FStopOnEntry: Boolean;       // Break at entry point
...
procedure TDAPServer.SetSourceMap(const ASourceMap: TSourceMap);
begin
    ...
    try
      try
        ...
        begin
          ...
          // Also log to FErrors for server-side diagnostics
          if Assigned(FErrors) then
...
var
  ...
  LThread: TJSONObject;
begin
  // Viper is single-threaded — report one thread
...
var
  ...
  LScope: TJSONObject;
begin
  // Phase 1: Return a single "Locals" scope (variables come in Phase 3)
  ...
  case LEvent.Reason of
    ...
    begin
      ...
      if LEvent.ExceptionMessage <> '' then
        ...
        LBody.AddPair('description', Format('Exception 0x%x', [LEvent.ExceptionCode]));
